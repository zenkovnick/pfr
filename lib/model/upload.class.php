...
{
    protected $options;
    private $image_types = array('image/gif', 'image/jpg', 'image/png', 'image/jpeg', 'image/pjpeg', 'image/x-png');
...
//                )
                'temp_image' => array(
                    'upload_dir' => dirname($_SERVER['SCRIPT_FILENAME']).'/uploads/temp/',
                    'upload_url' => $this->getFullUrl().'/uploads/temp/',
                    'max_width' => 400,
                    'max_height' => 500
                )
            )
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index) {
        $file = new stdClass();
        $file->name = md5(date("d-m-Y H-M-s")).".".pathinfo($name, PATHINFO_EXTENSION);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                if ($this->create_scaled_image($file->name, $this->options['image_versions']['temp_image']))
                {
                    $file->size = filesize($this->options['image_versions']['temp_image']['upload_dir'].$file->name);
                    $file->url = $this->options['upload_url'].$file->name;
                }
                else
                {
                    $file->error = 'compression error';
                }
    ...
    private function resizeToWidth($curr_size, &$size) {
        $ratio = $curr_size[1] / $curr_size[0];
        $size['height'] = ceil($size['height'] * $ratio);
    }
